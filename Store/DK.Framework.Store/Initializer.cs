...
namespace DK.Framework.Store
{
    ...
    public static class Initializer
    {
        ...
        public static void ResetWith(IEnumerable<Assembly> partAssemblies, IEnumerable<Type> partTypes)
        {
            ...
            Requires.IsTrue(partAssemblies.Any(), "Assemblies parameter is empty.");
            
            var configuration = new ContainerConfiguration();

            configuration = configuration.WithAssemblies(partAssemblies);

            if (null != partTypes)
            {
                configuration = configuration.WithParts(partTypes);
            }
